...
public class PredatorAgent extends Agent {
    ...
    private class PredatorBehaviour extends CyclicBehaviour {
        ...
        public void action() {
            ...
            // Lose energy every 4 cycles (very slow hunger)
            if (cycleCount % 4 == 0) {
                energy -= ENERGY_LOSS;
            }
            ...
            // Check if too crowded with other predators
            if (nearbyPredators.size() > 4) {
                ...
                // Try to catch - ONLY ONE PREY AT A TIME
                if (minDist <= CATCH_DISTANCE) {
                    capture(target);
                    justAte = true;
                    eatingCooldown = EATING_COOLDOWN;
                } else {
                    // Chase - use dynamic speed
                ...
                // No prey or still digesting: try reproduction or wander
                ...
                // Always keep moving - explore for prey - use dynamic speed
            ...
            // Slow down after eating (but not too much)
            try {
        ...
        private void capture(AgentInfo prey) {
            ...
            jade.lang.acl.ACLMessage killMsg = new jade.lang.acl.ACLMessage(
            ...
            System.out.println("🦁 " + getLocalName() + " ate prey (E:" + energy + ")");
        }
        ...
        private void reproduce() {
            ...
            try {
                ...
                System.out.println("🐶 " + getLocalName() + " had baby");
            } catch (Exception e) {}
        }
